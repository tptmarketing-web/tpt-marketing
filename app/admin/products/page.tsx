...
const selectClass =
  'w-full sm:w-auto px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-semibold text-gray-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-yellow-400 cursor-pointer';
...
export default function AdminProductsPage() {
  ...
  const visible = useMemo(() => {
    ...
    if (status !== 'all') {
      list = list.filter((p) => (status === 'active' ? p?.isActive : !p?.isActive));
    }
    ...
    return list;
  }, [products, search, category, format, status, sort]);

  const hasFilters =
    search.trim() !== '' || category !== 'all' || format !== 'all' || status !== 'all' || sort !== 'default';
    ...
    <div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4 mb-6">
        <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-end gap-3">
          <div className="flex flex-col gap-1 w-full sm:w-72">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
              />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide pl-1">Status</label>
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide pl-1">Sort by</label>
                ...
                {visible.map((product) => (
                  <tr key={product._id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      {hasFilters ? (
                        <span className="text-xs text-gray-400" title="Clear filters to reorder">#{product?.sortOrder ?? 0}</span>
                      ) : (
